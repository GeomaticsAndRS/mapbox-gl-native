...
enum class ExpressionSubclass : int32_t {
    Coalesce,
    CompoundExpression,
    Literal,
    ArrayAssertion,
...
class Expression {
public:
    Expression(ExpressionSubclass subclass_, type::Type type_) : subclass(subclass_), type(std::move(type_)) {}
    ...
    ExpressionSubclass getSubclass() const { return subclass; };
