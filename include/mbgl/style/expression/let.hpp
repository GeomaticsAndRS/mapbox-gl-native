...
public:
    ...
    Let(Bindings bindings_, std::unique_ptr<Expression> result_) :
        Expression(ExpressionSubclass::Let, result_->getType()),
    ...
    bool operator==(const Expression& e) const override {
        if (e.getSubclass() == ExpressionSubclass::Let) {
...
public:
    Var(std::string name_, std::shared_ptr<Expression> value_) :
        Expression(ExpressionSubclass::Var, value_->getType()),
        name(std::move(name_)),
        value(value_)
    {}
    ...
    bool operator==(const Expression& e) const override {
        if (e.getSubclass() == ExpressionSubclass::Var) {
